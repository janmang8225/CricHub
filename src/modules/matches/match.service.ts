...
export async function completeMatchService(matchId: string) {
  ...
  try {
    const match = await db.query(
      `SELECT status, team_a_id, team_b_id, toss_winner_team_id, toss_decision 
    ...
    const secondInningsBattingTeamId = 
      firstInningsBattingTeamId === m.team_a_id
        ? m.team_b_id
        : m.team_a_id;
...
// change2
export async function getPlayingXIService(matchId: string) {
  const result = await db.query(
    `SELECT 
      mxi.team_id,
      t.name as team_name,
    ...
    ORDER BY mxi.team_id, mxi.is_captain DESC, mxi.is_vice_captain DESC`,
    [matchId]
  );
...
// change1
export async function setPlayingXIService(
  matchId: string,
  teamId: string,
  players: Array<{
    playerId: string;
  ...
  try {
    ...
    for (const p of players) {
      await db.query(
        ...
        [matchId, teamId, p.playerId, p.isCaptain, p.isViceCaptain, p.isWicketKeeper, p.isSubstitute]
      );
    }
...
// change2
export async function listMyMatchesService(userId: string, role: string, page: number, limit: number) {
  const offset = (page - 1) * limit;
