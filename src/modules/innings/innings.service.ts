...
export async function switchInningsService(matchId: any, actor: any) {
  ...
  try {
    ...
    // debug - fix1
    const matchRes = await db.query(
      `SELECT status, team_a_id, team_b_id, toss_winner_team_id, toss_decision FROM matches WHERE id=$1`,
      [matchId]
    );
