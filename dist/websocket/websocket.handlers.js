 ...
 */
export function handleClientMessage(client, message, roomManager) {
    try {
        switch (message.type) {
            case 'join_match':
 ...
 */
function handleJoinMatch(client, matchId, userId, roomManager) {
    if (!matchId) {
 ...
 */
function handleLeaveMatch(client, matchId, roomManager) {
    if (!matchId) {
        sendError(client, 'matchId is required');
        return;
    }
