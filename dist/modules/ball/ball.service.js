...
export async function submitBallService(matchId, actor, ball) {
    ...
    try {
        ...
        // get innings
        const scoreRes = await db.query(`SELECT MAX(innings) AS innings FROM scores WHERE match_id=$1 LIMIT 1`, [matchId]); // added MAX(innings) (extra check)
        ...
        const lastBallRes = await db.query(`SELECT over, ball, is_wide, is_no_ball
       FROM balls
       WHERE match_id=$1 AND innings=$2
       ORDER BY created_at DESC, over DESC, ball DESC
       LIMIT 1`, [matchId, currentInnings]);
        let currentOver = 1;
        let currentBall = 1;
      ...
      `, [matchId, currentInnings, ball.over]);
        ...
        if (typeof ball.isWide !== 'boolean')
            throw new Error("Invalid isWide");
        if (typeof ball.isNoBall !== 'boolean')
            throw new Error("Invalid isNoBall");
        if (typeof ball.isBye !== 'boolean')
            throw new Error("Invalid isBye");
        if (typeof ball.isLegBye !== 'boolean')
            throw new Error("Invalid isLegBye");
        if (typeof ball.isPenalty !== 'boolean')
            throw new Error("Invalid isPenalty");
        if (typeof ball.isFreeHit !== 'boolean')
            throw new Error("Invalid isFreeHit");
        if (typeof ball.isWicket !== 'boolean')
            throw new Error("Invalid isWicket");
    ...
    `, [
        ...
        const legalBallsRes = await db.query(`SELECT COUNT(*) as legal_balls
       FROM balls
       WHERE match_id=$1 AND innings=$2 AND is_wide=false AND is_no_ball=false`, [matchId, currentInnings]);
        const legalBalls = parseInt(legalBallsRes.rows[0].legal_balls);
        const overs = Math.floor(legalBalls / 6);
        const balls = legalBalls % 6;
        const oversDecimal = overs + (balls / 10);
        await db.query(`UPDATE scores
       SET overs = $1
       WHERE match_id=$2 AND team_id=$3`, [oversDecimal, matchId, battingTeamId]);
        // ============ STEP 25: UPDATE STRIKER BATTING STATS ============
        await db.query(`UPDATE match_batting_stats
       SET runs = runs + $1,
           balls = balls + $2,
           fours = fours + $3,
           sixes = sixes + $4
       WHERE match_id=$5 AND player_id=$6`, [
        ...
        await db.query(`UPDATE match_bowling_stats
       SET balls = balls + $1,
       ...
       WHERE match_id=$6 AND player_id=$7`, [
        ...
        const partnershipRes = await db.query(`SELECT * FROM partnerships
       WHERE match_id=$1 AND innings=$2
        ...
        if (partnershipRes.rowCount === 0) {
            ...
            await db.query(`INSERT INTO partnerships 
        ...
        if (ball.isWicket) {
            ...
            await db.query(`UPDATE match_batting_stats
         SET is_out = true
         WHERE match_id=$1 AND player_id=$2`, [matchId, ball.dismissedPlayerId]);
            // STEP 35: Insert fall of wickets
            const currentScoreRes = await db.query(`SELECT runs, wickets FROM scores WHERE match_id=$1 AND team_id=$2`, [matchId, battingTeamId]);
            const currentScore = currentScoreRes.rows[0];
            await db.query(`INSERT INTO fall_of_wickets 
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
            ...
            if (ball.fielderId && ['CAUGHT', 'RUN_OUT', 'STUMPED'].includes(ball.wicket)) {
                let fielderRole;
                if (ball.wicket === 'CAUGHT')
                    fielderRole = 'CATCH';
                else if (ball.wicket === 'STUMPED')
                    fielderRole = 'STUMPING';
                else if (ball.wicket === 'RUN_OUT') {
                    fielderRole = ball.isDirectRunOut ? 'DIRECT_RUN_OUT' : 'RUN_OUT_ASSIST';
                }
                if (fielderRole) {
                    await db.query(`INSERT INTO dismissal_fielders 
             (match_id, innings, over, ball, fielder_id, role)
             VALUES ($1, $2, $3, $4, $5, $6)`, [matchId, currentInnings, ball.over, ball.ball, ball.fielderId, fielderRole]);
                }
            }
         ...
         WHERE match_id=$3 AND innings=$4
           ...
           AND end_over IS NULL`, [
            ...
            if (ball.wicket === 'RUN_OUT') {
             ...
             SET striker_id = $1,
                 non_striker_id = NULL,
                 updated_at = now()
             WHERE match_id=$2 AND innings=$3`, [survivingBatsmanId, matchId, currentInnings]);
                }
                else {
           ...
           SET striker_id = NULL,
               updated_at = now()
           WHERE match_id=$1 AND innings=$2`, [matchId, currentInnings]);
            }
        ...
        if (!ball.isWicket) {
            ...
            const overBallsRes = await db.query(`SELECT COUNT(*) as legal_balls
         FROM balls
         WHERE match_id=$1 AND innings=$2 AND over=$3 
           ...
           SET striker_id = $1,
               non_striker_id = $2,
               updated_at = now()
           WHERE match_id=$3 AND innings=$4`, [ball.nonStrikerId, ball.strikerId, matchId, currentInnings]);
            }
        }
        ...
        // Get current score
        const currentScoreRes = await db.query(`SELECT runs, wickets, overs FROM scores 
       WHERE match_id=$1 AND team_id=$2`, [matchId, battingTeamId]);
        const currentScore = currentScoreRes.rows[0];
        ...
        if (inningsEnded) {
         ...
         WHERE match_id=$3 AND innings=$4 AND end_over IS NULL`, [ball.over, ball.ball, matchId, currentInnings]);
            // STEP 60-61: Calculate and set target (1st innings only)
            if (currentInnings === 1) {
                const target = currentScore.runs + 1;
                await db.query(`UPDATE matches
           SET target_score = $1
           WHERE id = $2`, [target, matchId]);
            }
        }
        ...
        for (const milestone of battingMilestones) {
            ...
            const strikerStatsRes = await db.query(`SELECT runs FROM match_batting_stats
         WHERE match_id=$1 AND player_id=$2`, [matchId, ball.strikerId]);
            if (strikerStatsRes?.rowCount &&
                ...
                if (strikerRuns >= milestone) {
                    ...
                    const existingMilestone = await db.query(`SELECT milestone_value FROM player_milestones
             WHERE match_id=$1 AND innings=$2 AND player_id=$3 
               AND milestone_type='RUNS'
             ORDER BY milestone_value DESC
             LIMIT 1`, [matchId, currentInnings, ball.strikerId]);
                 ...
                 VALUES ($1, $2, $3, 'RUNS', $4, $5, $6)`, [matchId, currentInnings, ball.strikerId, milestone, ball.over, ball.ball]);
                        ...
                        if (previousRuns < milestone && milestone > currentMilestoneValue) {
                            await db.query(`UPDATE player_milestones
        ...
        for (const milestone of bowlingMilestones) {
            const bowlerStatsRes = await db.query(`SELECT wickets FROM match_bowling_stats
         WHERE match_id=$1 AND player_id=$2`, [matchId, ball.bowlerId]);
            if (bowlerStatsRes?.rowCount &&
             ...
             ORDER BY milestone_value DESC
             LIMIT 1`, [matchId, currentInnings, ball.bowlerId]);
                    if (existingMilestone.rowCount === 0) {
        ...
        if (ball.isWicket) {
            ...
            const last3BallsRes = await db.query(`SELECT is_wicket, over, ball
         FROM balls
         WHERE match_id=$1 AND innings=$2 AND bowler_id=$3
         ORDER BY over DESC, ball DESC
         LIMIT 3`, [matchId, currentInnings, ball.bowlerId]);
            if (last3BallsRes.rowCount === 3) {
                ...
                if (allWickets) {
                    ...
                    // );
                    const existingHatTrick = await db.query(`SELECT 1 FROM player_milestones
             WHERE match_id=$1 AND innings=$2 AND player_id=$3 
               AND milestone_type='HAT_TRICK'`, [matchId, currentInnings, ball.bowlerId]);
        ...
        const latestStateRes = await db.query(`SELECT striker_id, non_striker_id
      FROM batting_state
      WHERE match_id=$1 AND innings=$2`, [matchId, currentInnings]);
        ...
        const strikerStatsRes = latestStrikerId
            ? await db.query(`SELECT runs, balls, fours, sixes
          FROM match_batting_stats
          WHERE match_id=$1 AND player_id=$2`, [matchId, latestStrikerId])
            : { rows: [] };
        const nonStrikerStatsRes = latestNonStrikerId
            ? await db.query(`SELECT runs, balls, fours, sixes
          FROM match_batting_stats
          WHERE match_id=$1 AND player_id=$2`, [matchId, latestNonStrikerId])
            : { rows: [] };
        const bowlerStatsRes = await db.query(`SELECT balls, runs_conceded, wickets
