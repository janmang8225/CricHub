...
export async function switchInningsService(matchId, actor) {
    ...
    const matchRes = await db.query(`SELECT status FROM matches WHERE id=$1`, [matchId]);
    ...
    // get innings
    const scoreRes = await db.query(`SELECT innings FROM scores WHERE match_id=$1 LIMIT 1`, [matchId]);
    const innings = scoreRes.rows[0].innings;
    if (innings !== 1) {
        throw new Error("Innings already switched");
    }
    // switching both team's innings (setting 2)
    await db.query(`UPDATE scores SET innings = 2, updated_at = now() WHERE match_id=$1`, [matchId]);
