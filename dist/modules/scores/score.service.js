...
export async function updateScoreService(matchId, teamId, runs, wickets, overs, actor) {
    ...
    try {
        // match must be LIVE
        const m = await db.query("SELECT status FROM matches WHERE id=$1", [matchId]);
        if ((m.rowCount ?? 0) === 0) {
        ...
        if (m.rows[0].status === 'COMPLETED') {
            const err = new Error("Cannot update scores for completed match");
            err.statusCode = 403;
            throw err;
        }
...
export async function getScoreService(matchId) {
    const result = await db.query(`
    SELECT team_id, runs, wickets, overs, innings
    FROM scores
    WHERE match_id=$1
    ORDER BY innings DESC
    `, [matchId]); // check (updated is_batting with innings)
    return result.rows;
}
export async function getCurrentInningService(matchId) {
    const result = await db.query(`SELECT team_id, MAX(innings) AS innings 
     FROM scores 
     WHERE match_id=$1
...
export async function startMatchService(matchId, tossWinnerTeamId, tossDecision) {
    // match must exist + CREATED
    const m = await db.query("SELECT status, team_a_id, team_b_id FROM matches WHERE id=$1", [matchId]);
    if (m.rowCount === 0)
        throw new Error("Match not found");
    if (m.rows[0].status !== "CREATED")
        throw new Error("Match not in CREATED state");
...
export async function getBattingStateService(matchId) {
    ...
    const battingTeamId = innings === 1
        ...
        : (match.toss_decision === "BAT"
            ? (match.team_a_id === match.toss_winner_team_id
                ...
                : match.team_a_id)
            : match.toss_winner_team_id);
    const bowlingTeamId = battingTeamId === match.team_a_id
        ? match.team_b_id
        : match.team_a_id;
    /* ---------------- Last ball ---------------- */
...
export async function setOpenersService(matchId, strikerId, nonStrikerId) {
    ...
    try {
        // Check match status
        const matchCheck = await db.query(`SELECT status FROM matches WHERE id=$1`, [matchId]);
        ...
        // ...
        const inningRes = await db.query(`SELECT MAX(innings) AS innings FROM scores WHERE match_id=$1`, [matchId]);
        const innings = inningRes.rows[0].innings;
        if (!innings)
            throw new Error("Scores not initialized");
        // prevent re-setting
        const exists = await db.query(`SELECT 1 FROM batting_state WHERE match_id=$1 AND innings=$2`, [matchId, innings]);
        if (exists.rowCount !== null && exists.rowCount > 0) {
            throw new Error("Openers already set");
        }
        // Get batting team
        const matchRes = await db.query(`SELECT team_a_id, team_b_id, toss_winner_team_id, toss_decision
       FROM matches WHERE id=$1`, [matchId]);
        ...
        const playerCheck = await db.query(`SELECT player_id FROM team_players 
       WHERE team_id = $1 AND player_id = ANY($2) AND is_active = true`, [battingTeamId, [strikerId, nonStrikerId]]);
        if (playerCheck.rowCount !== 2) {
            throw new Error("Both openers must belong to batting team and be active");
        }
        // Set batting state
        await db.query(`INSERT INTO batting_state (match_id, innings, striker_id, non_striker_id)
       VALUES ($1, $2, $3, $4)`, [matchId, innings, strikerId, nonStrikerId]);
        // Initialize batting stats for striker
        await db.query(`INSERT INTO match_batting_stats (match_id, player_id, team_id, runs, balls, fours, sixes, is_out)
       VALUES ($1, $2, $3, 0, 0, 0, 0, false)
       ON CONFLICT (match_id, player_id) DO NOTHING`, [matchId, strikerId, battingTeamId]);
...
export async function setNextBatsmanService(matchId, newBatsmanId) {
    // change1
    await db.query("BEGIN");
    try {
        // Check match status
       ...
       FROM matches WHERE id=$1`, [matchId]);
        ...
        const playerCheck = await db.query(`SELECT 1 FROM team_players 
       WHERE team_id = $1 AND player_id = $2 AND is_active = true`, [battingTeamId, newBatsmanId]);
        if (playerCheck.rowCount === 0) {
            throw new Error("New batsman must belong to batting team and be active");
        }
        // replace striker only (dismissed batsman)
        await db.query(`UPDATE batting_state
       SET striker_id = $1, updated_at = now()
       WHERE match_id=$2 AND innings=$3`, [newBatsmanId, matchId, innings]);
...
export async function setBowlerService(matchId, over, bowlerId) {
    // change1
    await db.query("BEGIN");
    try {
        // Check match status
        ...
        // ...
        // get match + innings
        const matchRes = await db.query(`SELECT m.team_a_id, m.team_b_id,
       ...
       GROUP BY m.id`, [matchId]);
        ...
        const battingTeamId = m.innings === 1
            ? (m.toss_decision === "BAT"
            ...
            : (m.toss_decision === "BAT"
                ? (m.team_a_id === m.toss_winner_team_id
                    ...
                    : m.team_a_id)
                : m.toss_winner_team_id);
        const bowlingTeamId = battingTeamId === m.team_a_id ? m.team_b_id : m.team_a_id;
        // validate bowler belongs to bowling team
        const player = await db.query(`SELECT 1 FROM team_players WHERE player_id=$1 AND team_id=$2 AND is_active=true`, [bowlerId, bowlingTeamId]);
        if (player.rowCount === 0) {
            throw new Error("Bowler not from bowling team or not active");
        }
        // prevent overwrite
        const exists = await db.query(`SELECT 1 FROM over_state
       WHERE match_id=$1 AND innings=$2 AND over=$3`, [matchId, m.innings, over]);
        if (exists.rowCount !== null && exists.rowCount > 0) {
            throw new Error("Bowler already set for this over");
        }
        // insert over_state
        await db.query(`INSERT INTO over_state (match_id, innings, over, bowler_id)
       VALUES ($1,$2,$3,$4)`, [matchId, m.innings, over, bowlerId]);
        // Initialize bowling stats for bowler
        await db.query(`INSERT INTO match_bowling_stats (match_id, player_id, team_id, balls, runs_conceded, wickets, wides, no_balls)
...
export async function updateStrikeService(matchId, strikerId, nonStrikerId) {
    if (strikerId === nonStrikerId) {
...
// change1
export async function completeOverManuallyService(matchId, over) {
    const inningRes = await db.query(`SELECT MAX(innings) AS innings FROM scores WHERE match_id=$1`, [matchId]);
    const innings = inningRes.rows[0].innings;
    if (!innings)
        throw new Error("Scores not initialized");
    // Check if summary already exists
    const exists = await db.query(`SELECT 1 FROM over_summary WHERE match_id=$1 AND innings=$2 AND over=$3`, [matchId, innings, over]);
    if (exists.rowCount !== null && exists.rowCount > 0) {
        throw new Error("Over summary already exists");
    }
    // Calculate stats for this over
    const overStatsRes = await db.query(`SELECT 
       SUM(runs_off_bat + extra_runs) as runs,
       COUNT(CASE WHEN is_wicket = true THEN 1 END) as wickets,
       SUM(extra_runs) as extras
     FROM balls
     WHERE match_id=$1 AND innings=$2 AND over=$3`, [matchId, innings, over]);
    const overStats = overStatsRes.rows[0];
    if (!overStats.runs) {
        throw new Error("No balls found for this over");
    }
    await db.query(`INSERT INTO over_summary 
...
export async function declareInningsService(matchId) {
    await db.query("BEGIN");
    try {
        // Check match status
        const matchCheck = await db.query(`SELECT status FROM matches WHERE id=$1`, [matchId]);
        ...
        // Get last ball
        const lastBallRes = await db.query(`SELECT over, ball FROM balls
       WHERE match_id=$1 AND innings=$2
       ORDER BY created_at DESC
       LIMIT 1`, [matchId, innings]);
        if (lastBallRes.rowCount === 0) {
            throw new Error("No balls bowled yet");
        }
        const lastBall = lastBallRes.rows[0];
        // End all open partnerships
        await db.query(`UPDATE partnerships
       SET end_over = $1,
           end_ball = $2,
           ended_reason = 'DECLARATION'
       WHERE match_id=$3 AND innings=$4 AND end_over IS NULL`, [lastBall.over, lastBall.ball, matchId, innings]);
        // Get current score and set target
        const scoreRes = await db.query(`SELECT runs FROM scores WHERE match_id=$1 AND innings=$2
       ORDER BY runs DESC LIMIT 1`, [matchId, innings]);
        const target = scoreRes.rows[0].runs + 1;
        await db.query(`UPDATE matches SET target_score = $1 WHERE id = $2`, [target, matchId]);
        await db.query("COMMIT");
    }
    catch (e) {
        await db.query("ROLLBACK");
...
export async function endInningsManuallyService(matchId, reason) {
    await db.query("BEGIN");
    try {
       ...
       LIMIT 1`, [matchId, innings]);
        ...
        const overSummaryExists = await db.query(`SELECT 1 FROM over_summary 
       WHERE match_id=$1 AND innings=$2 AND over=$3`, [matchId, innings, lastBall.over]);
       ...
       SET end_over = $1,
           end_ball = $2,
           ended_reason = $3
       WHERE match_id=$4 AND innings=$5 AND end_over IS NULL`, [lastBall.over, lastBall.ball, reason, matchId, innings]);
...
export async function getMilestonesService(matchId) {
    const result = await db.query(`SELECT 
     ...
     WHERE pm.match_id = $1
     ORDER BY pm.innings, pm.created_at`, [matchId]);
    return result.rows;
}
